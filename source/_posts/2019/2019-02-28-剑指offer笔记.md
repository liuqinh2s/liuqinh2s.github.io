...
- 数组的空间效率不如链表，可能存在闲置的空间
- 数组在内存中是连续的，查找复杂度是O(1)，也由于这个特性可以用来做简单的哈希表
- 数组插入和删除复杂度是O(n)

>动态数组的做法：当数组满的时候，申请一块两倍于当前数组大小的内存空间，将原数组空间中的所有元素移到新空间，然后释放原数组内存空间。根据分摊分析，插入每个元素的时间复杂度最多是3（3个普通操作，普通操作的意思是：比如插入，移动元素，其实质都是给内存空间赋值）。[动态数组的分摊分析](../../../../2019/03/03/动态数组的分摊分析)
...
- 链表动态分配内存，不会存在闲置的内存，所以空间效率比数组高
- 链表的内存不连续，查找是O(n)的复杂度，而数组是O(1)的复杂度
