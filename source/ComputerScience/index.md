...
计算机科学发展到现在，现如今编程自然不用纸带+打孔，裸机跑程序。我们的工作都是建立在前人的基础之上。让我们看看我们现在要学习的是什么：
...
为什么 ACM 如此重要（以及 ACM 的大奖对本科生找工作的分量这么重），因为编程活动所要依赖的只有两个个：抽象和逻辑，这就是为什么其实编程可以从小学开始（没有过高的知识门槛）。计算机科学的诸多课程，就是由先辈们靠着抽象和逻辑搭建起来的知识大厦，这些知识和经验是叶，而你抽象和逻辑能力，依靠计算机解决问题的能力是根。练习 ACM 就跟武术世界里面的李小龙每天练拳一样，是基本功。
